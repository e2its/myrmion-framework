...
El Marco Regulatorio 1.4 de Consultora Modelo declara aplicable el EU AI Act. El caso de uso «dictamen jurídico interno» está clasificado en el Marco (§2.2) como **riesgo limitado**: no figura en las áreas del Anexo III — ese es el criterio de clasificación, no la presencia de supervisión humana. Que el dictamen lo firme y decida un humano (Riera) es una salvaguarda del diseño que el Marco registra, no lo que desclasifica el caso. El rol es **deployer**: el agente corre sobre un modelo comercial sin modificación sustancial ni marca propia (art. 25 revisado con el DPO). El Marco exige la ficha de transparencia para este caso de uso (genera contenido que llega a personas identificado como asistido por IA), así que se referencia; al no ser alto riesgo, `impactAssessmentRef` y `pmmPlanRef` no son exigibles. La clasificación la aprobó el custodio del Marco el 2026-02-01; si el caso de uso cambiara, la reclasificación re-dispararía el gate.
